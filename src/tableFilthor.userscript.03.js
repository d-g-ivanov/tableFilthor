...
(function() {
	...
	const DEFAULT_OPTIONS = {
        ...
        refresh: {
            ...
            shouldDiff: false,
            columnToDiffBy: null,
            pre: [function startSpinner(table) { table.panel.querySelector('[data-action=refresh]').style.animation = 'table-filthor-spin 4s linear infinite'; }],
            post: [function stopSpinner(table) { table.panel.querySelector('[data-action=refresh]').style.animation = ''; }],
    ...
    const ACTIONS = {
        ...
        refreshSettings: function refreshSettings(event, table) {
            createRefreshModal(event, table);
        },
        // clear all filters
        ...
        // change refresh options
        saveRefreshOptions: function saveRefreshOptions(event, modal, table) {
    ...
    const REFRESH = {
        start: async function start(table) {
            ...
            if(opts && typeof opts === 'object' && opts.post && Array.isArray(opts.pre)) {
            ...
            const time = new Date()
            table.panel.querySelector('.table-filthor-last-refresh').textContent = time.toLocaleTimeString(); // last refresh date
            table.panel.querySelector('.table-filthor-last-refresh').title = `Last updated: ${time.toLocaleTimeString()}`; // last refresh date

            // post refresh functions
            if(opts && typeof opts === 'object' && opts.post && Array.isArray(opts.post)) {
        ...
        addRows: function addRows(target, source) {
            const toClone = source.tBodies.length ? [...source.tBodies] : [...source.rows];
            const cloned = toClone.map( el => el.cloneNode(true) );

            target.table.append(...cloned);
        },
        removeRows: function removeRows(table) {
            const toRemove = table.table.tBodies.length ? table.table.tBodies : table.tr;
        ...
        diff: function diff(table, newTable, columnToDiffBy) {
            ...
            if (typeof columnToDiffBy !== 'number') columnToDiffBy = table.headers.indexOf(columnToDiffBy);
            if (columnToDiffBy === -1) {
            ...
            for (let i = 0, len = oldRows.length; i < len; i++) {
                ...
                if (map[value]) {
                    ...
                    for( let cellIndex = 0, cellsLen = row.cells.length; cellIndex < cellsLen; cellIndex++) {
                        ...
                        if (cell.textContent === newCell.textContent) {
                            ...
                            // MARK_UPDATE
                            if (typeof diffOptions.markUpdate === 'function') diffOptions.markUpdate(cell, oldCellClone, newCell);
                ...
                // if map does not contain the value: REMOVED ROW
                    ...
                    // MARK_REMOVED
                    if (typeof diffOptions.markRemoval === 'function') diffOptions.markRemoval(row);
            ...
            if (remainingIndices.length) {
                for (let i = 0, len = remainingIndices.length; i < len; i++) {
                    ...
                    // MARK_ADDITION
                    if (typeof diffOptions.markAddition === 'function') diffOptions.markAddition(row);
...
<div class="modal-box">
  ...
  <div class="modal-footer">
    <button class="close-modal">Close!</button> ${content.actionButtons ? content.actionButtons.join('') : ''}
  </div>
</div>`
        };
...
.table-filthor-summaries a {
  ...
  text-align: center;
    ...
    function createRefreshModal(event, table) {

        const modal = {
            header: '<h3>Set up your refreshes</h3>',
            body:
...
.table-filthor-refresh-save:hover { opacity: 0.7; }`,
            actionButtons: [
                '<button class="table-filthor-refresh-save" data-action="saveRefreshOptions">Save</button>'
            ]
        };
...
`<div class="table-filthor-actions">
   <a class="filthor-icon" type="button" data-action="refresh" title="Manually re-fetch table from server">${ASSETS.refresh}</a>
   <a class="filthor-icon" type="button" data-action="refreshSettings" title="Update refresh settings">${ASSETS.settings}</a>
...
<div class="table-filthor-stats">
   <a type="button" data-action="stats" title="See more stats">${table.shown} of ${table.tr.length} rows</a>
</div>
</td>`;
    ...
    function updatePresets(table) {
        ...
        if (autorefreshStore) {
            if (autorefreshStore.autorefresh) table.options.refresh.autorefresh = autorefreshStore.autorefresh;
            if (autorefreshStore.autorefreshRate) table.options.refresh.autorefreshRate = autorefreshStore.autorefreshRate;
            if (autorefreshStore.shouldDiff) table.options.refresh.shouldDiff = autorefreshStore.shouldDiff;
            if (autorefreshStore.columnToDiffBy) table.options.refresh.columnToDiffBy = autorefreshStore.columnToDiffBy;
    ...
    function createOptions(opts) {
		...
		if (opts) {
            ...
            options.refresh = opts.refresh ? Object.assign({}, DEFAULT_OPTIONS.refresh, opts.refresh) : Object.assign({}, DEFAULT_OPTIONS.refresh);
            if( (opts.refresh.pre || opts.refresh.post) && options.refresh.keepDefaultHooks) {
                options.refresh.pre = (opts.refresh.pre || []).concat(DEFAULT_OPTIONS.refresh.pre);
                options.refresh.post = (opts.refresh.post || []).concat(DEFAULT_OPTIONS.refresh.post);
            }
        } else {
    ...
    function prepTables(selector, options) {
		...
		tables.forEach((table, i) => {
            ...
            // update opts with data from local storage - filters, summary types, refresh (coming)
            updatePresets(t);
